...
import { user, account as accountTable } from '$lib/server/db/schema';
			...
			const existingUser = await db.query.user.findFirst({
				where: (users, { and, eq, ne }) => 
					and(
						...
						ne(users.id, locals.session!.user.id)
					)
			});
	...
	changePassword: async ({ request, locals, platform }) => {
		...
		try {
			...
			const currentUser = await db.query.user.findFirst({
				where: (users, { eq }) => eq(users.id, locals.session!.user.id)
			});
			...
			const account = await db.query.account.findFirst({
				where: (accounts, { and, eq }) => 
					and(
						eq(accounts.userId, locals.session!.user.id),

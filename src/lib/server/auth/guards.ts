...
export function hasFieldPermission(entityType: string, fieldName: string, userRole: Role): boolean {
...
export function getSecurityHeaders(): Record<string, string> {
	return {
		...
		'Content-Security-Policy':
			"default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval'; style-src 'self' 'unsafe-inline'; img-src 'self' data: https:; font-src 'self' data:; connect-src 'self' https://api.telegram.org"
	};
}
